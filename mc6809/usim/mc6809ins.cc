...
{
	br(!(cc.bit.n ^ cc.bit.v));
}

void mc6809::lbge(void)
{
	lbr(!(cc.bit.n ^ cc.bit.v));
}

void mc6809::bgt(void)
{
	br(!(cc.bit.z & (cc.bit.n ^ cc.bit.v)));
}

void mc6809::lbgt(void)
{
	lbr(!(cc.bit.z & (cc.bit.n ^ cc.bit.v)));
}

void mc6809::bhi(void)
{
	br(!(cc.bit.c | cc.bit.z));
}

void mc6809::lbhi(void)
{
	lbr(!(cc.bit.c | cc.bit.z));
}
...
void mc6809::invalid(void)
{
	fprintf(stderr, "Invalid instruction (%02x) at PC:%04x\n", ir, pc);
	halted = 1;
}
