...
private:
	...
	Byte&			byterefreg(int);
	Word&			wordrefreg(int);
	Word			read_word(Word);	// Read word
	void			write_word(Word, Word);
	Byte			fetch(void);		// Read [PC] and increment
	Byte			fetch_operand(void);
	Word			fetch_word(void);	// Read [PC] and increment
	...
	void			invalid();		// Invalid instruction
...
virtual	void			write(Word, Byte);	// Write byte
virtual	Byte			read(Word);		// Read byte

public:
				mc6809(void);		// public constructor
	virtual			~mc6809(void);		// public destructor

	void			reset(void);		// CPU reset
	void			execute(void);		// Fetch and execute an instruction
	void			run(void);		// Run continuously
	void			load(const char *filename);	// Load INTEL hexdump
