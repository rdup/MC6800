...
mc6809::mc6809(void) : a(acc.byte.a), b(acc.byte.b), d(acc.d)
{
...
{
	write(addr, val >> 8);
	write(addr + 1, (Byte)val);
}

// Fetch a byte from the current PC and increment PC
Byte mc6809::fetch(void)
{
	return read(pc++);
}

// Fetch a 2 byte word from memory
Word mc6809::fetch_word(void)
{
	Word	ret = 0;
...
void mc6809::load(const char *filename)
{
	FILE		*fp;
	int		done = 0;
...
{
	switch (post & 0x9f) {
		case 0x82: refreg(post) -= 1; break;
		case 0x83: refreg(post) -= 2; break;
	}
}

Word mc6809::extend5(Byte ex)
{
	if (ex & 0x10) {
		return (Word)ex | 0xfff0;
	} else {
		return (Word)ex;
	}
}

Word mc6809::extend8(Byte ex)
{
	if (ex & 0x80) {
		return (Word)ex | 0xff00;
	} else {
		return (Word)ex;
	}
}

void mc6809::br(int test)
{
	Word offset = extend8(fetch());
	if (test) pc += offset;
}

void mc6809::lbr(int test)
{
	Word offset = fetch_word();
	if (test) pc += offset;
}
